...
//        System.out.println("init() 호출 되었다 ");
//    }

    @Override
    public void doGet(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
        System.out.println("doget()호출 ( 매요청마다 호출  ");
        resp.setContentType("text/html;charset=UTF-8");
        PrintWriter out = resp.getWriter();
        out.print("<h1> 게시글 작성</h1>");
        out.print("<form method='POST' action='board'>");
        out.print("분류1: <input type=checkbox name= category value ='분류1> 분류2'");
        out.print("분류2: <input type=checkbox name= category value ='분류2> 분류2'");
        out.print("분류3: <input type=checkbox name= category value ='분류3> 분류3'");
        out.print("제목: <input type=text name=subject><br>");
        out.print("내용: <textarea name='content'></textarea><br>");
        out.print("<button type='submit'>작성 하기</button>");
        out.print("</form>");
